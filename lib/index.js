 ...
 * WaifuX stores static wallpapers in Wallpapers/ and motion-video wallpapers
 * in Media/; ~/Documents/dsh/we-content is for loose files dropped manually. */
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      if (st.isDirectory()) {
        ...
        // macOS WaifuX layout: each workshop item lives one level deeper under
        // Media/workshop_<id>/steamapps/workshop/content/431960/<id>/. Scan
        // that nested content dir exactly like a bare content root.
        const nested = join(dir, 'steamapps', 'workshop', 'content', WE_APPID);
        if (!existsSync(nested)) continue;
        let nestedEntries = [];
        try { nestedEntries = readdirSync(nested); } catch { continue; }
        for (const sub of nestedEntries) {
          const subDir = join(nested, sub);
          let subSt; try { subSt = statSync(subDir); } catch { continue; }
          if (!subSt.isDirectory()) continue;
          const subProj = readProject(subDir);
          if (!subProj || found.has(subProj.id)) continue;
          subProj.fileAbs = subProj.type === 'scene'
            ? (() => { const main = resolveSceneMainFile(subDir, subProj.file); return resolve(subDir, main || subProj.file); })()
            : resolve(subDir, subProj.file);
          subProj.previewAbs = subProj.preview ? resolve(subDir, subProj.preview) : null;
          found.set(subProj.id, subProj);
        }
        continue;
      }
      ...
      // half renders an img element; playable flags come from the same
      // existsSync path as project media.
